...
Nuclear Physics B (2022). arXiv:2111.02536 [hep-ph].}
...
LHCb measurements ($3.1\,\sigma$ tension):
              \begin{itemize}
\item $R_{K^+} = 0.846^{+0.042}_{-0.039}{}^{+0.013}_{-0.012}$ \colorcite{R. Aaij \textit{et al.} (LHCb) arXiv:2103.11769},
\item $R_{K^{*0}} = 0.685^{+0.113}_{-0.069}\pm0.047$ \colorcite{R. Aaij \textit{et al.} (LHCb) arXiv:1705.05802}.
              \end{itemize}
...
Combined tension of $3.08\,\sigma$.
...
$\Delta M_S^\mathrm{SM} = 20.01 \pm 1.25\,\mathrm{ps}^{-1}$, \qquad$\Delta M_S^\mathrm{exp} = 17.757 \pm 0.021 \,\mathrm{ps}^{-1}$. \\ \colorcite{{L. Di Luzio, M. Kirk, A. Lenz. arXiv:1712.06572}}
...
\item CP asymmetry: $\ACPmix (B_s\to J/\psi \phi)= \sin \left[\mathrm{arg}\left(   1 + \frac{C_{bs}^{LL} }{R^\mathrm{loop}} \right) - 2 \beta_s \right] $.
              \end{itemize}
    \end{itemize}
    ...
    \begin{itemize}
\item $Z'$: $\mathcal{L}_{Z'} = \frac{1}{2} M_{Z'}^2 (Z'_\mu)^2 + (\lambda_{ij}^Q \bar{d}^i_L \gamma^\mu d^j_L + \lambda_{\alpha\beta}^L \bar{e}^\alpha_L \gamma^\mu e_L^\beta )Z'_\mu $.
\item Leptoquarks ($S_3$): $\mathcal{L}_{S_3} = -M^2_{S_3} |S_3^a|^2 + y_{i\alpha}^{QL} \bar{q^c}^i (\epsilon \sigma^a) \ell^\alpha S_3^a  + \mathrm{h.c.}$.
    \end{itemize}
...
$$R_{K^{(*)}}\qquad\qquad\qquad\qquad\qquad\qquad R_{D^{(*)}}$$
    ...
    \end{figure}
    \begin{itemize}
\item $C_{\ell q}^e \approx - C_{\ell q}^\mu$ required for $R_{K^{(*)}}$.
...
Measure of the importance of each parameter in the Machine Learning prediction for a single datapoint \colorcite{S. Lundberg, S. Lee. arXiv:1705.07874}.
...
We follow the Lepage-Brodsky technique \colorcite{G. Lepage, S. J. Brodsky, Phys. Rev. D 22 (1980), p. 2157.}:
    \begin{itemize}
...
\begin{frame}[plain]
    \vspace{1.3cm}
    \begin{block}{\huge General conclusions}
        Chapter 9
    \end{block}
    \vspace{1.3cm}
    \begin{itemize}
...
\begin{frame}\frametitle{Conclusiones generales}
    \begin{itemize}
...
\begin{frame}\frametitle{Conclusiones generales}
    ...
    \begin{itemize}
        ...
        \item Necesidad de realizar análisis globales para dar cuenta de todos los efectos posibles de la Nueva Física.
        \item Aplicabilidad de técnicas de \textit{Machine Learning} para ayudar en los estudios fenomenológicos.
    \end{itemize}
\end{frame}
